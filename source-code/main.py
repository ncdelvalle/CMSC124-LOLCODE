...
# Global Vars
root = None
defaultDir = os.getcwd()   # use current directory
...
def program():
    ...
    if current_token is not None and current_token[1] == "var_declaration_start":  # WAZZUP
        ...
        varDeclarationList = var_section_stmt()
...
def statement_list():
    ...
    while True:
        ...
        if not valid_statement(current_token):
            break
...
def function_call():
    global current_token
    global current_line, symbol_table
...
# Functions
def console_print(msg, end="\n"):
    console.config(state="normal")
    console.insert(tk.END, str(msg) + end)
    console.see(tk.END)
...
def refresh_dir():
    ...
    for name in names:
        ...
        if os.path.isfile(full) and name.lower().endswith(".lol"):
...
def execute_code():
    ...
    console.delete("1.0", tk.END)
     ...
     # Display lexemes
    if tokens:
        for lex, cls in tokens:
            lexemeTable.insert("", "end", values=(lex.replace("\n", "\\n"), cls))
    else:
        lexemeTable.insert("", "end", values=("NO TOKENS", ""))

    program()

    # Refresh symbol table using NEW FUNCTION
    refresh_symbol_table()
...
def create_gui():
    ...
    # ← Removed sampleCode insertion (editor starts empty)
